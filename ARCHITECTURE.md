...
- **Производство (`production`)** — роль «только файлы», как проектировщик: сделки и сервис не редактирует.
- **Админ** — редактирует справочники (позиции, цены, пользователи) через Django admin. Единственная роль, которая может удалять сделки (в т.ч. через API умника).
- **Руководитель** — видит всю воронку, аналитику, может править любые сделки.

## 3. Ключевые концепции

### Проект, сделка, версия
- **Deal (Сделка)** — основная сущность. Один клиент — одна сделка. Имеет `project_code` вида `3МД-Иванов-Пулково` (`{модули}МД-{клиент}-{участок}`, уникальный бизнес-ключ).
- **ServiceRequest (Обращение)** — рекламация / доработка / вопрос после сдачи. Живёт в `/service/`, не в воронке `Deal.status`. См. [docs/service-requests.md](docs/service-requests.md).
...
ServiceRequest (обращение после сдачи)
  - number                       уникальный SR-n
  - deal, client                 необязательные FK
  - kind                         'reclamation'|'service'|'question'
  - status                       'new'|'in_progress'|'waiting'|'done'|'rejected'
  - priority, source, assignee, resolution
...
- **DB:** PostgreSQL 16
- **Frontend:** Django templates + HTMX + Alpine.js для интерактивности на старте. Конфигуратор с живым пересчётом возможно переедет на React (решаем в фазе 5).
- **Файловое хранилище:** локальная папка `/var/crm_files/`, через `django-storages` можно будет переключить на S3/MinIO.
- **Аутентификация:** Django стандартная для людей, DRF token для плагина.
- **Деплой:** Docker Compose (app + Postgres + nginx + certbot для HTTPS).
...
- Чат умника в UI виден любому активному залогиненному пользователю (`can_use_umnik_chat`). Он ходит в `POST {UMNIK_URL}/crm/chat`.
- Обратный вызов умника в CRM: `/api/umnik/…` с тем же `UMNIK_TOKEN` и заголовком `X-Umnik-Actor` (username; по умолчанию `admin`). Меняются статус, наценка, конфигуратор и суммы; права — от пользователя-actor, не «всегда admin». Контракт: [docs/umnik-crm-api.md](docs/umnik-crm-api.md). Вложения чата: [docs/umnik-chat-attachments.md](docs/umnik-chat-attachments.md).

## 6. Ключевые бизнес-правила (что легко забыть)

1. `project_code` — уникальный, регистронезависимый для поиска (`project_code_normalized`). Конструктор кода: `{модули}МД-{клиент}-{участок}` (`build_project_code_from_parts`).
2. `module_count` — целое число 0..15 в UI и API умника; плагин ArchiCAD требует 1..15. Не ссылка на каталог.
...
v1.1 — уточнены роли (`production`), формат `project_code`, чат умника для всех залогиненных,
входящий `/api/umnik/…` и отдельный контур `ServiceRequest`. Базовый текст — v1.0 (до кодирования).