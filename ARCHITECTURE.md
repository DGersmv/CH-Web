...
- **Deal (Сделка)** — основная сущность. Один клиент — одна сделка. Имеет `project_code` вида `3МД-Иван-Пулково` (уникальный бизнес-ключ; автосборка `{модули}МД-{клиент}-{участок}`).
...
Операционный цикл (лиды, claim, статусы, вкладки этапов): `docs/deal-lifecycle.md`.

### Количество модулей
Метка "3МД", "5МД", "7МД" и т.д. означает количество модулей в доме. Хранится как `module_count` на Deal (`0..15` в UI/модели; plugin API — `1..15`). Это **не** ссылка на каталог моделей — каталога нет, каждый дом индивидуальный. Используется только для фильтрации и аналитики.
...
1. `project_code` — уникальный через `project_code_normalized` (lowercase + схлопнутые пробелы). Автоформат `{модули}МД-{клиент}-{участок}`.
2. `module_count` — целое число 0..15 в CRM UI; plugin ingest требует 1..15. Не ссылка на каталог.
